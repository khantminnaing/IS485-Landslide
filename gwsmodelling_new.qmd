...
```{r}
pacman::p_load(sp, sf, st, spdep, raster, spatstat, tmap, devtools,vtable,ggplot2,egg, corrplot, patchwork, ggstats, ggstatsplot, GWmodel, tidyverse, gtsummary,vtable, sjPlot, sjmisc, sjlabelled, tableHTML, olsrr, car, blorr,ISLR, klaR, rsample,kableExtra, performance)
```
...
```{r}
GLM <- gwlr$glms
residuals.sf <- cbind(training_data_sf, residuals = GLM$residuals)
st_crs(residuals.sf)
residuals.sp <- as(residuals.sf, "Spatial")
```

```{r}
ggplot(residuals.sf, aes(x=residuals)) +
  geom_histogram(color="white")+
  labs(title="Residuals of Generalised Linear (Logistic) Regression Model", x="Residuals", y="Count")  +
  coord_cartesian(xlim = c(-50, 2900))
```
...
tm_shape(valtellina)+
  tm_polygons()+
tm_shape(residuals.sf)+
  tm_dots(col = "residuals", palette ="RdYlGn",  style="quantile", midpoint = 0) +
  tm_layout(
    main.title = "Residuals of Generalised Linear (Logistic) Regression Model",
    main.title.size = 1,
...
SDF <- gwlr$SDF
SDF.sf <- st_as_sf(SDF)
SDF.sf <- st_set_crs(SDF.sf, 32632) 
st_crs(SDF.sf)
SDF.sp <- as(SDF.sf, "Spatial")
```

```{r}
ggplot(SDF.sf, aes(x=residual)) +
  geom_histogram(color="white") +
  labs(title="Residuals of Geographically Weighted Logistic Regression Model", x="Residuals", y="Count")  +
...
tm_shape(valtellina)+
  tm_polygons()+
tm_shape(SDF.sf)+
  tm_dots(col = "residual", midpoint = NA, palette ="RdYlGn",  style="quantile", auto.palette.mapping=FALSE) +
  tm_layout(
    main.title = "Residuals of Geographically Weighted Logistic Regression Model",
    main.title.size = 1,
    main.title.position = "center",
    legend.position = c("right", "bottom"))
```

### 4.4.4 Moran's $I$ for Residuals

#### Moran's $I$ of LR Residuals

```{r}
coords <- coordinates(residuals.sp)
...
```{r}
dnear_nb_95 <- dnearneigh(coords, 0, 95, longlat = FALSE)
listw <- nb2listw(dnear_nb_95, style="W", zero.policy=TRUE)
moran.test(residuals.sp$residuals, listw)
```

```{r}
dnear_nb_198 <- dnearneigh(coords, 0, 198, longlat = FALSE)
listw <- nb2listw(dnear_nb_198, style="W", zero.policy=TRUE)
moran.test(residuals.sp$residuals, listw)
```

```{r}
knear <- knearneigh(coordinates(residuals.sp), k=20) 
knear_nb <- knn2nb(knear)
listw <- nb2listw(knear_nb, style="W")
moran.test(residuals.sp$residuals, listw)
```

#### Moran's $I$ of GWLR Residuals

```{r}
dnear_nb <- dnearneigh(coordinates(SDF.sp), 0, 2922, longlat = FALSE)
listw <- nb2listw(dnear_nb, style="W")
moran.test(SDF.sp$residual, listw)
```

```{r}
knear <- knearneigh(coordinates(SDF.sp), k=20) 
knear_nb <- knn2nb(knear)
listw <- nb2listw(knear_nb, style="W")
moran.test(SDF.sp$residual, listw)
```
