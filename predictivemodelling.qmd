---
title: "Predictive Landslide Susceptibility Modelling Using Machine Learning Techniques" 
author:
...
Landslide susceptibility modelling is an emerging field of research aiming to identify the contributing factors of landslides and to predict the most susceptible areas. These studies serve as a valuable decision-support tool to facilitate the identification of the highest risk areas and implement preventive measures accordingly. In this research project, we aim to demonstrate the potential contribution of spatial non-stationarity in landslide susceptibility modelling. To do so, we will attempt to make a comparative case study of nonspatial and spatial random forest techniques, and how space can be incorporated to these models. In particular, we will be developing three different RF models
...
The code chunk below performs this operation. This process is crucial because it allows us to train our model on a subset of the data (the training set) and then evaluate its performance on unseen data, the testing set. This ensures that our model is not just memorising the training data and is actually able to generalise as new data. The **`set.seed(1243)`** function ensures that the random split of data is reproducible. The **`initial_split`** function from the **`rsample`** package is used to perform the split, with 20% of the data allocated to the testing set (**`prop = .2`**). The split is stratified by the **`Landslide`** variable, meaning the proportion of each class of **`Landslide`** is the same in the training and testing sets.
...
## 3.2 Checking the Balance of Classes
...
## 3.3 Preparing the Testing Data

In this section, we are further refining our testing data for the analysis. Here, we perform two main operations:
...
-   We extract the 'X' and 'Y' columns from the training data and store them in a new data frame called `xy`.
...
## 3.5 Transforming Data to Simple Feature Objects
...
## 3.6 Distance Matrix Transformation and Threshold Setting
...
After examining the quantile values, we set the rounded distance thresholds as follows:
...
To address this issue, we use Moran's I, a common measure of spatial autocorrelation widely adopted by the geospatial data science community. Moran's I quantifies the similarity between values and neighbouring observations, considering spatial relationships between neighbours. It describes how each observation differs from the mean (or average) of the study area.

Our aim is to assess the spatial autocorrelation level of predictor variables at different spatial scales. The function **`plot_training_df_moran()`** assesses the spatial autocorrelation of the response variable and the predictors across different distance thresholds. This function generates a plot that visualises the spatial autocorrelation of the response variable and the predictors across different distance thresholds. The **`fill.color`** and **`point.color`** arguments are used to customise the colors of the plot.
...
By default, RF model from `ranger` uses the following parameters:
...
The **`randomForestExplainer`** package offers several options to deepen our understanding of variable importance scores. One such option is the **`measure_importance()`** function, which analyses the forest to find out:
...
-   The number of times the variable was selected first to start a tree (**`times_a_root`**).
...
The **`rf_importance()`** function from the **`spatialRF`** package provides a way to assess the extent to which each predictor contributes to the model transferability. Model transferability refers to the model's predictive ability on independent spatial folds, which can be measured with the **`rf_evaluate()`** function (discussed later).
...
-   **RMSE** and its normalised version: These are computed via the **`root_mean_squared_error()`** function, and are linear with R squared and pseudo R squared.
...
Borcard and Legendre developed an approach called the Principal Coordinates of Neighbour Matrices (PCNM) to create spatial predictors that can be incorporated into models. Dray and Legendre further \[2006\] improved mathematical foundations of PCNM and investigated its close relation to Moran's index of spatial autocorrelation (Moran's I). They proposed a Moran's eigenvector approach which extracts Moran's Eigenvector Maps (MEMs) from the spatial weighting matrix.

The spatial weight matrix, denoted as W= $w_i$ $_j$ , is an n×n matrix that represents the spatial relationships between different locations of n observations in the dataset. Each entry in the matrix represents the spatial relationship between location i and location j. The centering operator H subtracts the mean of each row and column from the elements of W, resulting in a doubly-centred weight matrix, Ω, where the row and column means are zero. By diagonalising this doubly-centred weight matrix Ω, eigenvectors obtained are orthogonal vectors with a unit norm that maximise Moran's *I*. These eigenvectors are called Moran's Eigenvector Maps (MEMs). The eigenvalues of MEMs are equal to the Moran's I coefficients of spatial autocorrelation and describe the local structures of the observations in the dataset.
...
As explained earlier, the **`rf_spatial`** function generates Moran's eigenvectors and includes them into the training dataset through sequential selection. The selection procedure is performed by the **`select_spatial_predictors_sequential()`** function, which finds the smaller subset of spatial predictors that maximises the model's R squared and minimises the Moran's I of the residuals. The **`rf_spatial`** function already automates this process, so there is no need to manually run it. However, we can use the **`plot_optimization()`** function to visualise the optimisation process. In the plot below, dots linked by lines represent the selected spatial predictors.
...
We can also visualise the spatial predictors obtained from the Moran's Eigenvector Random Forest (MERF) model. This can be done using the **`get_spatial_predictors()`** function from the **`spatialRF`** package, which retrieves the spatial predictors from the MERF model.
...
We can visualise these importance scores using the **`plot_importance()`** function from the **`spatialRF`** package, print them with the **`print_importance()`** function, or retrieve the data frame of importance scores with the **`get_importance()`** function.
...
The **`step`** parameter is used to define the increment in the bandwidth during the optimisation process. The **`bw.min`** and **`bw.max`** parameters define the range within which the optimal bandwidth is searched. The **`geo.weighted`** parameter is a boolean flag that determines the type of random forest algorithm to use. If **`geo.weighted`** is set to **`TRUE`**, the algorithm calculates a Geographically Weighted Random Forest using the **`case.weights`** option of the **`ranger`** package. This means that each observation in the local dataset is weighted based on its geographical location. If **`geo.weighted`** is set to **`FALSE`**, it will calculate local random forests without weighting each observation in the local dataset. Given that our study is interested in spatial nonstationarity, **`geo.weighted`** is set to **`TRUE`**.
...
## 7.3 Prediction with Various Spatial Weights
...
-   Predictions are made using GWRF model with specific bandwidth and weight parameter.
...
## 7.4 Plotting GWRF Model Prediction Accuracy at Incrementing Weight & Optimised Bandwidth
...
## 7.5 Iteraction with Standardised Bandwidths
...
## 7.6 Prediction with Various Spatial Weights
...
## 7.7 Plotting GWRF Model Prediction Accuracy at Incrementing Weight & Standardised Bandwidth
...
## 7.8 Bandwidth and Spatial Weight Selection

Georganos & Kalogirou (2022) observed accuracy results generated from models of lower bandwidths are unstable since the model uses fewer samples. As such, they calibrated models and concluded bandwidths between 100 and 400 are more reliable. Balancing between initial $R^2$ performances and the above-rendered accuracy scores, the local weight of 0.75 for bandwidth 189 is selected for the interpolation of landslide susceptibility prediction. 
...
In this code chunk, we create a **`gstat`** object named **`k`** using the **`gstat`** function. This object represents a geostatistical model that we use for Kriging. The formula **`Predicted ~ 1`** specifies that we are modelling the **`Predicted`** variable as a function of a constant (i.e., we're fitting a mean model). The **`data`** argument is set to **`testing_data_sf`**. The **`model`** argument is set to **`fv`**.
...
## 8.7 Rasterising Predictions

In this code chunk, we rasterise the predictions **`resp`** onto the raster grid **`grid`** using the **`terra::rasterize`** function. The rasterised predictions are stored in **`kpred`**.
