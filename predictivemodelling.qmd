...
Random Forest (RF) is a stochastic process, and to ensure that the results are not random, we can perform a permutation test. This involves repeating the RF process multiple times and comparing the results. In this case, we will repeat the process 30 times using the **`rf_repeat()`** function from the **`spatialRF`** package.
...
tm_shape(kpred) + 
  ...
  tm_layout(main.title = "Distribution of Predicted Landslides",
